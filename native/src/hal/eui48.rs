...
pub enum ParseError {
  ...
  InvalidLength(usize),
  /// The input string is invalid, usize bytes were found, and we put up to 6
  /// bytes into Eui48
...
impl MacAddress {
  ...
  ///
  /// Returns an error (without any description) if the slice doesn't have the
  /// proper length.
  ...
  /// Returns a String representation in the EUI-64 interface ID format
  /// '0000:00ff:fe00:0000'
  ...
  /// Returns a String representation in the IPv6 link local format
...
mod tests {
  ...
  fn test_is_local() {
    ...
    assert_eq!("06:34:56:ab:cd:ef", local.to_hex_string()); // Catch modifying
                                                            // first octet
  }
